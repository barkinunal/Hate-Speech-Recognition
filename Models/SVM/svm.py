...
def feature_extract(train_text, test_text, stop_word=True):
    ...
    if stop_word:
        tf_idf = TfidfVectorizer(max_df=0.95, ngram_range=(1, 1), stop_words="english")
    ...
    if not stop_word:
        tf_idf = TfidfVectorizer(max_df=0.95, ngram_range=(1, 1))
        tf_idf.fit_transform(train_text)
...
for b in [True, False]:
    ...
    SVM.fit(train_feature_set, binary_train)

    predictions = SVM.predict(test_feature_set)
